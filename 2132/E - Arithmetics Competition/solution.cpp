...
int main(){
	...
	TESTCASE{
		...
		//print("size",prea.size(),preb.size());
		REP(q){
			cin >>x>>y>>z;
			pair<int,int> p = dp[z];
...
";
			ll sum=0;
			if(p.first>x){
				if(z-x>0)sum+=preb[z-x-1];
				if(x>0)sum+=prea[x-1];
			}
			 else if(p.second>y){
				if(y>0)sum+=preb[y-1];
				if(z-y>0)sum+=prea[z-y-1];
			} else {
			if(p.first>0)sum+=prea[p.first-1];
			if(p.second>0)sum+=preb[p.second-1];}
			cout << sum << "
";
			//print(prea[p.first-1],preb[p.second-1]);
			}
	}
}
 
