...
 
        ...
        vector<vector<ll>> dp(m+1,vector<ll>(totalh+1,-1));
        ...
        for(ll i=1;i<=m-1;i++){
            ...
            for(ll j=0;j<=totalh;j++){
                ...
                ll temphp = v[i-1][1];
                if(dp[i+1][j+temphp]==-1){
                    dp[i+1][j+temphp]=newcost;
                } else {
                    dp[i+1][j+temphp]=min(newcost,dp[i+1][j+temphp]);
                }
 
            }
        }
        vector<ll> ans;
        for(ll j=0;j<=totalh;j++){
            if(dp[m][j]+v[m-1][0]<=(m-1)*x && dp[m][j]>=0){
                ans.pb(j+v[m-1][1]);
            } else if(dp[m][j]>=0){
                ans.pb(j);
            }
