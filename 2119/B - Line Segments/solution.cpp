...
int main(){
	...
	TESTCASE{
		...
		float px,qx,py,qy;
		cin >> n >> px >> py >> qx >> qy;
		float distance = pow((px-qx)*(px-qx)+(py-qy)*(py-qy),0.5);
		vec<int> v;
		readvector(v,n);
		if(n==1){
			if(abs(v[0]-pow((px-qx)*(px-qx)+(py-qy)*(py-qy),0.5))<pow(10,-6)){
				cout << "Yes
";
			} else {
				cout << "No
";
			...
			float largest = MAX_OF(MAX_IN(v),distance);
			ll sum = SUM(v);
			if(sum>=distance){
				if(distance+sum-largest>=largest){
					cout << "Yes
";
				} 
				else {
