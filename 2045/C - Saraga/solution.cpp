// Problem: C. Saraga
// Contest: Codeforces - 2024-2025 ICPC Asia Jakarta Regional Contest (Unrated, Online Mirror, ICPC Rules, Teams Preferred)
...
int main(){
	...
	for(int i=1;i<s.size();i++){
		if(!m.count(s[i])){
			...
			for(int j=t.size()-2;j>=0;j--){
				if(s[i]==t[j]){
					if(t.size()-j+i<leastsize){
						leastsize=t.size()-j+i;
						indices={i,j};
					}
					break;
				}
	...
	if(leastsize==pow(10,7))cout << "-1
";
	else {
		for(int i=0;i<=indices.first;i++) cout << s[i];
		for(int j=indices.second+1;j<t.size();j++)cout << t[j];
		cout << "
";
	}
}
