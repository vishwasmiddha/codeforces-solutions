...
int main(){
    speed();
        ...
        for(int i=1;i<n;i++){
            for(int j=0;j<k;j++){
                regions[i][j]=regions[i][j]|regions[i-1][j];
            }
        }
        REP(q){
            ll m;cin>>m;
            // explain m requirements and adjust feasible region
            int left=0,right=n-1;
            bool possible=true;
            FOR(turn,m){
                //print("turn",turn,left,right);
                ll region,compare;
                char symb;
                cin >> region;
                cin>>symb;
                cin>>compare;
                region--;
                //print(region,symb,compare);
                if(symb=='<'){
                    // find largest country with region < compare
                    int left2=left,right2=right;
                    while(left2<right2){
                        int mid = (left2+right2+1)/2;
                        if(regions[mid][region]<compare){
                            left2=mid;
                        } else {
                            right2=mid-1;
                        }
                    ...
                    right=right2;
                    if(regions[left][region]>=compare)possible=false;
                } else {
                    // find smallest country with region > compare
                    int left2=left,right2=right;
                    while(left2<right2){
            ...
            if(possible && left<=right)cout << left+1<<"
";
            else cout << "-1
";
        }
