...
 
void dfs(int node, vector<bool> &visited, vector<ll>&attack, vector<vector<ll>> &dp,vector<vector<int>>&edges){
    visited[node]=true;
    vector<int> child;
    for(auto c:edges[node]){
        if(!visited[c]){child.pb(c);
        dfs(c,visited,attack,dp,edges);
        }
    }
    if(child.size()==0){
        for(ll i=1;i<=20;i++)dp[node][i]=i*attack[node];
    } else {
        ll num = child.size();
        vector<vector<pair<ll,ll>>> v2(num,vector<pair<ll,ll>>());
        for(int i=0;i<child.size();i++){
            for(int j=1;j<=20;j++){
                v2[i].pb({dp[child[i]][j],j});
            }
            SORT(v2[i]);
            for(int j=1;j<=18;j++)v2[i].pop_back();
        }
        for(ll i=1;i<=20;i++){
            // turn i;
            ll cost= i*attack[node];
            for(int j=0;j<num;j++){
                if(v2[j][0].second==i)cost += v2[j][1].first;
                else cost+=v2[j][0].first;
 
            }
            dp[node][i]=cost;
        }
