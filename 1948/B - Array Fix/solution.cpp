...
bool solve(vector<int> &v, int n){
	...
	bool breaking = true;
	for(int i=0;i<n;i++){
		if(v[i]<10)smallone=i;
	}
	for(int i=0;i<n;i++){
		if(i<=smallone){
			if(v[i]<10){
				if(v[i]<lastnum)return false;
				else lastnum=v[i];
			}
			else {
				if(v[i]/10<lastnum)return false;
				else {
					lastnum = v[i]/10;
					if(lastnum>v[i]%10)return false;
					else lastnum = v[i]%10;
				}
			}
		} else if (breaking){
			if(v[i]/10<lastnum){
				...
				if(v[i]%10<v[i]/10)breaking=false;
				else lastnum = v[i]%10;
			}
			if(!breaking){
				if(lastnum>v[i])return false;
				else lastnum = v[i];
			}
		} else {
			if(lastnum>v[i])return false;
