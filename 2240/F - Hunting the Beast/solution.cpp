...
 
    ...
    TESTCASE{
        ll n,m;cin>>n>>m;
        ll ans1 = power(n-1,n)%MOD;
        ll ans2=0;
        for(ll i=1;i<=n-m;i++){
            ll temp = ncr(n-m,i,fact,invfact);
            ll temp2=power(n-1-i,n-1);
            ll temp3 = (temp%MOD)*(temp2%MOD);
            temp3=temp3%MOD;
            if(i%2==1){
                ans2 = (ans2-temp3+MOD)%MOD;
            } else {
                ans2 = (ans2+temp3)%MOD;
            }
        }
        ans2 = ((n-1)*ans2)%MOD;
        ll sum = (ans1+ans2)%MOD;
        ll incl = (ncr(n,m,fact,invfact)*sum);
        cout << (incl)%MOD<<"
";
    }
}
