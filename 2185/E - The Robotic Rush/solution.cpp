...
 
        SORT(b);
        vector<vector<ll>> c;
        for(int i=0;i<n;i++)c.pb({a[i],i,1});
        for(int i=0;i<m;i++)c.pb({b[i],i,2});
        priority_queue<vector<ll>> leftdeath;
        priority_queue<vector<ll>> rightdeath;
        SORT(c);
        queue<ll> robotpos;
        ll leftspike = -1;
        for(auto c2:c){
            if(c2[2]==1){
                if(leftspike>=0){
                    leftdeath.push({leftspike-c2[0],c2[1]});
                }
                robotpos.push(c2[1]);
            } else {
                while(!robotpos.empty()){
                    ll temp=robotpos.front();
                    robotpos.pop();
                    rightdeath.push({a[temp]-c2[0],temp});
 
                }
                leftspike=c2[0];
            }
        }
        vector<bool> aliveguys(n,true);
        ll alive=n;
        ll position=0;
        string moves;
        cin>>moves;
        for(auto c2:moves){
            if(c2=='R'){
                position++;
                while(!rightdeath.empty()){
                    vector<ll>temp2= rightdeath.top();
                   // print(temp2[0],"death",position,"right");
                    if(-temp2[0]==position){
                        if(!aliveguys[temp2[1]]){
                            rightdeath.pop();
                            continue;
                        }
                        alive--;
                        aliveguys[temp2[1]]=false;
                        rightdeath.pop();
                    }
                    else break;
 
            ...
            position--;
            //print("leftscene",leftdeath.size());
                while(!leftdeath.empty()){
                    vector<ll>temp2= leftdeath.top();
                  //  print(temp2[0],"death",position,"left");
                    if(temp2[0]==position){
                        if(!aliveguys[temp2[1]]){
                            leftdeath.pop();
                            continue;
