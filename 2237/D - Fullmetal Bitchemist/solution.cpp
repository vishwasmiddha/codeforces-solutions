...
int main(){
    ...
    TESTCASE{
        ll n;cin>>n;
        string s;cin>>s;
        vector<vector<vector<ll>>> dp(n,vector<vector<ll>>(3,vector<ll>(3,0)));
        if(s[0]=='0'){
            dp[0][1][0]=1;
        } else {
            dp[0][0][1]=1;
        ...
        for(int i=0;i<n;i++){
            if(prev!=1-(s[i]-'0')){
                if(altleng>=3){
                    if(altleng%2==1){
                        sum2 += (altleng/2LL)*(altleng/2LL);
                    } else {
                        sum2 += (altleng/2LL-1)*(altleng/2LL);
                    }
                }
                prev=s[i]-'0';
                altleng=1;
       ...
       // print(sum,dp2[n-1][0][0],dp2[n-1][1][1],n);
      // print(sum,sum2);
        cout << sum - sum2 << "
";
    }
