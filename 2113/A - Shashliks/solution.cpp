...
int main(){
	...
	TESTCASE{
		...
		cin >> k >> a >> b >> x >> y;
		vec<int> req = {a,b};
		vec<int> decrease = {x,y};
		int least=pow(10,9)+1;
		int ind=-1;
		for(int i=0;i<=1;i++){
			if(least>decrease[i]){
				ind=i;
		...
		int amount=0;
		if(k>=req[ind]){
			amount+=(k-req[ind])/decrease[ind]+1;
			//print("increase",(k-req[ind])/decrease[ind]+1,k);
			k-=((k-req[ind])/decrease[ind]+1)*decrease[ind];
