...
 
        ...
        multiset<ll> mons;
        REP(m)mons.insert(0);
        vector<bool> night(l+1,false);
        for(auto c:a)night[c]=true;
        int pending=n;
        // mons should be <= pending+1
        for(int i=1;i<=l;i++){
            auto small = mons.begin();
            mons.erase(small);
            mons.insert(*small+1);
            
            if(night[i]){
                pending--;
                auto big=prev(mons.end());
                mons.erase(big);
                //print("deleted,",big);
            if(mons.size()+1<=pending+1){
                mons.insert(0);
            }
        }
        }
        //print("size",mons.size());
        cout << *mons.rbegin() << "
";
 
    }    
