...
int main(){
	...
	TESTCASE{
		...
		vector<int> leafdegree(n+1,0);
		int maxleafdeg = -1;//int ind=-1;
		int currleafdeg=0;
		for(int i=1;i<=n;i++){
			for(auto c:adj[i]){
				if(neigh_count[c]==1){
			...
			if(neigh_count[i]==1)currleafdeg++;
			if(currleafdeg>maxleafdeg){
				maxleafdeg=currleafdeg;
			}
			currleafdeg=0;
		}
		int leaves=0;
		
		for(auto c:neigh_count){
			if(c==1)leaves++;
		}
		cout << leaves-maxleafdeg << "
";
		//print(leaves,maxleafdeg);
	}
}
 
 
