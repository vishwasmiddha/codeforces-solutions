...
int main(){
    ...
    TESTCASE{
        ...
        vector<int> change1, change2, nochange, changeboth;
        if(s1[0]==s2[0]){
            change1.pb(INT_MAX/2);
            change2.pb(INT_MAX/2);
            nochange.pb(0);
            changeboth.pb(2);
        } else {
            change1.pb(1);
            change2.pb(1);
            nochange.pb(INT_MAX/2);
            changeboth.pb(INT_MAX/2);
        }
        for(int i=1;i<s1.size();i++){
            if(s1[i]=='0' && s2[i]==s2[i-1]){
                nochange.pb(min(change1[i-1],nochange[i-1]));
                change1.pb(min(change2[i-1],changeboth[i-1])+1);
                change2.pb(min(change2[i-1],changeboth[i-1])+1);
                changeboth.pb(min(change1[i-1],nochange[i-1])+2);
            } else if (s1[i]=='1' && s2[i]!=s2[i-1]){
                nochange.pb(min(change1[i-1],nochange[i-1]));
                change1.pb(min(change2[i-1],changeboth[i-1])+1);
                change2.pb(min(change2[i-1],changeboth[i-1])+1);
                changeboth.pb(min(change1[i-1],nochange[i-1])+2);
                ...
                nochange.pb(min(change2[i-1],changeboth[i-1]));
                change1.pb(min(change1[i-1],nochange[i-1])+1);
                change2.pb(min(change1[i-1],nochange[i-1])+1);
                changeboth.pb(min(change2[i-1],changeboth[i-1])+2);
            }
        }
        int n=s1.size();
       // printvec(nochange);
        cout << min(min(change1[n-1],change2[n-1]),min(nochange[n-1],changeboth[n-1]))<<"
";
    }    
}