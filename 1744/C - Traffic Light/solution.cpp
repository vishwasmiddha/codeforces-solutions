...
";
		...
		int start=-1,end=-1,current=0,highest=0;
		for(int i=0;i<n;i++){
			if(s[i]=='g'){start=i;break;}
		}
		for(int i=n-1;i>-1;i--){
			if(s[i]==c)end=i;
			if(s[i]=='g')break;
		...
		for(int i=0;i<n;i++){
			if(s[i]==c){
				if(current==0){current=1;highest=max(highest,current);}
				else{
					current++;
		...
		int highest2=-1;
		if(end!=-1&&start!=-1){
			highest2 = start + n - end;
		}
		cout << max(highest,highest2)<<"
";
	}
}
 
