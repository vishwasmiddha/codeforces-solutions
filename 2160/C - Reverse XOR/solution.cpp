...
";
            ...
            REVERSE(bools);
            while(bools.size()!=0 && bools[bools.size()-1]==0){
                bools.pop_back();
            }
            bool possible = true;
            if(bools.size()%2==1){
                if(bools[bools.size()/2]!=0)possible=false;
            }
            for(int i=0;i<bools.size();i++){
                if(bools[i]!=bools[bools.size()-i-1]){possible=false;break;}
            }
            boolprint(possible);
        }
    }
