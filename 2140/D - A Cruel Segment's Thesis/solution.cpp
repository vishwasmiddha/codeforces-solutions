...
int main(){
	...
	TESTCASE{
		...
		for(int i=0;i<n;i++){
			...
			left.pb(l);right.pb(r);diff.pb({-r-l,l,r});
			sum += 2*r-l;
		}	
		//diff[mostind]={1000000000000,diff[mostind][0],diff[mostind][1]};
		sort(diff.begin(),diff.end());
		REVERSE(diff);
		if(n==1){
			// print(diff[0][0],diff[0][1],diff[0][2]);
			cout << diff[0][2]-diff[0][1]<<endl;
			continue;
		}
		if(n%2==0){
		for(int i=0;i<n/2;i++){
