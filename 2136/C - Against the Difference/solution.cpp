...
 
int dp(vector<int>&a,int n, vector<vector<int>>&indices,vector<int>&dpsum){
	if(n==1)return a[0]==1;
	if(n<=0)return 0;
	if(dpsum[n]!=-1)return dpsum[n];
	if(a[n-1]>n){ dpsum[n]=dp(a,n-1,indices,dpsum);return dpsum[n];}
	int a1=0,a2=0;
	if(!indices[a[n-1]].empty()){
	indices[a[n-1]].pop_back();
	a1=dp(a,n-1,indices,dpsum);
	indices[a[n-1]].push_back(n-1);}
	vector<int> ind;
	if(indices[a[n-1]].size()>=a[n-1]){
		REP(a[n-1]){ind.pb(indices[a[n-1]][indices[a[n-1]].size()-1]);indices[a[n-1]].pop_back();}
		a2=dp(a,ind[ind.size()-1],indices,dpsum)+a[n-1];
		for(int i=ind.size()-1;i>=0;i--){
			indices[a[n-1]].pb(ind[i]);
		}
		ind.clear();
	}
	dpsum[n]=max(a1,a2);
...
int main(){
	...
	TESTCASE{
		...
		for(int i=0;i<n;i++){
			...
			if(a[i]<=n){indices[a[i]].pb(i);
			}
			
		}
		vector<int> dpsum(n+1,-1);
			dpsum[1]=(a[0]==1);
			dpsum[0]=0;
		cout<<dp(a,n,indices,dpsum)<<"
";
	}
}
 
