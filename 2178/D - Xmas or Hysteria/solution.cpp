...
";
            ...
            ll ind=n-2;
            ll temphp = v[n-1].first;
            while(ind>=0 && temphp>0){
                temphp-=v[ind].first;
                ind--;
            }
            if(temphp>0)cout << "-1
";
                ...
                for(int i=0;i<=ind;i++){
                    moves.pb({v[i].second,v[i+1].second});
                }
                for(int i=ind+1;i<n-1;i++){
                    moves.pb({v[i].second,v[n-1].second});
                }
                
            }
            if(moves.size()>0)cout << moves.size() << "
";
            for(auto c:moves)cout << c.first << " " << c.second << "
";
        } else {
            vector<bool> kill(n,true);
            kill[n-1]=false;
            ll tosave = m;
            tosave--;
            for(ll i=1;i<=tosave;i++){
                kill[n-1-2*i]=false;
            }
            for(ll i=0;i<n;i++){
                if(kill[i]){
                    moves.pb({v[i+1].second,v[i].second});
                }
            }
            cout << moves.size() << "
";
