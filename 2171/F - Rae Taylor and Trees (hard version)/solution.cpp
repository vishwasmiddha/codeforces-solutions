    ...
    TESTCASE{
        ...
        for(int i=n-1;i>-1;i--){
            if(foundone){
                ...
                if(bigboy==-1){possible = false;break;}
                foundone = true;
            } else {
                bigboy=max(bigboy,v[i]);
                edges.pb({1,v[i]});
            }
        }
        //printvec(pending);
        if(possible && (pending.size()==0)){
            cout << "Yes
";
            for(auto c:edges){
