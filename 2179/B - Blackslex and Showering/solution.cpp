...
int main(){
    ...
    TESTCASE{
        int n;cin>>n;
        vector<int> v,v2,diff;
        readvector(v,n);
        for(int i=1;i<n;i++)diff.pb(abs(v[i]-v[i-1]));
        v2.pb(abs(v[1]-v[0]));
        v2.pb(abs(v[n-1]-v[n-2]));
        for(int i=1;i<n-1;i++){
            v2.pb(abs(v[i]-v[i-1])+abs(v[i]-v[i+1])-abs(v[i-1]-v[i+1]));
        }
        cout << SUM(diff)-max(MAX_IN(v2),0)<<"
";
 
    }    
}