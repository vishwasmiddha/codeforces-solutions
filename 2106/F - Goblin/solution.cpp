...
 
void recurse(string &s, ll &topsize, ll &downsize, ll &maxsize, int &ind, int n){
	if(ind==n){
		return;
	}
	if(s[ind]=='1'){
		topsize=downsize+1;
		downsize=0;
		maxsize=max(topsize,max(downsize,maxsize));
	} else {
		topsize+=ind;
		downsize+=n-ind-1;
		maxsize=max(topsize,max(downsize,maxsize));
	}
	//print(ind,topsize,downsize);
	ind++;
	recurse(s,topsize,downsize,maxsize,ind,n);
}
 
int main(){
    speed();
	TESTCASE{
		...
		string s;
		cin >> s;
		ll upsize=0, downsize=0, maxsize=0;
		int ind = 0;
		recurse(s,upsize,downsize,maxsize,ind,n);
		cout << maxsize << "
";
	}
}
