    ...
    TESTCASE{
        ...
        while(!leaf.empty()){
           ...
           // print("front",temp);
            ...
            degree[temp]--;
            degree[temp2[0]]--;
            if(degree[temp2[0]]==1)leaf.push(temp2[0]);
            x=temp2[1];
            y=temp2[2];
            if((temp<temp2[0] && x<y)||(temp>temp2[0] && x>y)){
                permutation[temp]=least;
                least++;
            } else{
                permutation[temp]=most;
                most--;
            }
        }
        ll n2=n;
        for(int i=1;i<=n;i++){
            if(permutation[i]){
                cout << permutation[i] << " ";
            } else cout << (n2*(n2+1))/2-SUM(permutation) << " ";
        }
        cout << "
";
