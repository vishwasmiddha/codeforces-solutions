...
int main(){
    ...
    TESTCASE{
        int n,k;cin>>n>>k;
        vector<int> big1(n+1,-1);
        vector<int> big2(n+1,-1);
        vector<int> index(k+1,-1);
        vector<int> v;
        vector<int> temp;
        readvector(v,n);
        for(int i=0;i<n;i++){
            temp = {big1[v[i]],big2[v[i]],i-index[v[i]]-1};
            SORT(temp);
            big1[v[i]]=temp[2];
            big2[v[i]]=temp[1];
            index[v[i]]=i;
        ...
        for(int i=1;i<=k;i++){
            temp={big1[i],big2[i],n-index[i]-1};
            //print(i,big1[i],big2[i],n-index[i]-1);
            SORT(temp);
            big1[i]=temp[2];
            big2[i]=temp[1];
        }
        ...
        for(int i=1;i<=k;i++){
            ...
            if(big1[i]==-1)big1[i]=0;
            if(big2[i]==-1)big2[i]=0;
            //print(i,big1[i],big2[i]);
            biggest = min(biggest,max(big1[i]/2,big2[i]));
        }
        cout << biggest << "
";
    }
