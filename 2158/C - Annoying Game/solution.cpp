...
int main(){
    ...
    TESTCASE{
        ...
        prefix.pb(a[0]);
        for(int i=1;i<n;i++){
            prefix.pb(max(prefix[i-1],0LL)+a[i]);
        }
        suffix.assign(n,0);
        suffix[n-1]=a[n-1];
        for(int i=n-2;i>=0;i--){
            suffix[i]=max(suffix[i+1],0LL)+a[i];
        }
        vector<ll> temp;
        if(k%2){
        for(int i=0;i<n;i++)temp.pb(prefix[i]+suffix[i]-a[i]+b[i]);
        } 
        else {
            for(int i=0;i<n;i++)temp.pb(prefix[i]+suffix[i]-a[i]);
        }
        print(MAX_IN(temp));
    }    
}