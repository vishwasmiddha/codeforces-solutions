...
// URL: https://codeforces.com/contest/2128/problem/0
// Memory Limit: 256 MB
// Time Limit: 1000 ms
 
...
int main(){
	...
	TESTCASE{
		...
		for(int j=0;j<n;j++){
			if(v[j]<=c){
				largestind=j;
				//print("largestind ",v[j]);
			}
			else break;
		}
		if(largestind!=-1){
		coins=n-largestind-1;
		//cout << "coins: " << coins << "
";
		int remaining = n-coins;
		for(int i=largestind;i>=0;i--){
			if(v[i]*multi<=c){
				//print(v[i],multi,"sorted");
