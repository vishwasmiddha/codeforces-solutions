...
int main(){
	...
	TESTCASE{
		...
		for(auto c:s){
			if(c=='A'){
				if(blackpos<blacks.size() && blacks[blackpos]==position)blackpos++;
				position++;
				if(blackpos<blacks.size() && blacks[blackpos]==position){
					blackpos++;
				} else {
					newblacks.pb(position);
				}
			} else {
				// print("start ",position);
				 if(blackpos<blacks.size() && position==blacks[blackpos])blackpos++;
				position++;
				while(blackpos<blacks.size() && position==blacks[blackpos]){
					blackpos++;
					position++;
				}
				// print("push",position);
		...
		cout << blacks.size()+newblacks.size()<<"
";
		for(auto c:blacks){
			newblacks.pb(c);
			//print("pushed",c);
