...
ll solve(string &s, char c){
    ...
    for(ll i=first;i<=last;i++){
        if(s[i]==c){
            ...
            if(cnt<=goods/2){badinds1+=i-first;bads1++;}
            else {
                badinds2+=i-first;bads2++;
            }
        }
    }
    ll sum1= -(bads1*(bads1-1))/2 + badinds1;
    ll sum2 = (last-first)*bads2 -(bads2*(bads2-1))/2 - badinds2;
    //print(bads1,bads2,badinds1,badinds2,sum1,sum2);
    return sum1+sum2;
 
 
}
