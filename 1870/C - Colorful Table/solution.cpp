...
int main(){
    ...
    TESTCASE{
        ...
        vector<int> v;
        readvector(v,n);
        vector<int> window(k+1,-1);
        vector<int> freq(k+1,0);
        int p1=0,p2=n-1;
        for(int current=1;current<=k;current++){
            while(p1<p2 && v[p1]<current)p1++;
            while(p1<p2 && v[p2]<current)p2--;
            if(p1!=p2){
                window[current]=(p2-p1+1)*2;
            } else if(v[p1]==current)window[current]=2;
            else window[current]=0;
        }
        for(auto c:v)freq[c]++;
        for(int i=1;i<=k;i++)if(freq[i]==0)window[i]=0;
        for(int i=1;i<=k;i++)cout << window[i] << " ";
        cout << "
";
    }    
}