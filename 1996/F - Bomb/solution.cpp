...
int main(){
    ...
    TESTCASE{
        ...
        readvector(a,n);
        readvector(b,n);
        // binary search over least number added.
        ll right = MAX_IN(a)+1LL;
        ll left = 1;
        while(left<right){
            ll mid = (left+right)/2;
            // find moves to reach mid.
            // mid is the least number i can add to my score.
            ll moves=0;
            for(int i=0;i<n;i++){
                if(a[i]>=mid){
                    ll times = (a[i]-mid)/b[i]; 
                    moves += times+1;
                    //else moves += times;
                }
                if(moves>k){
        ...
        for(int i=0;i<n;i++){
            if(a[i]>=finalvalue){
                ll times = ((a[i]-finalvalue)/b[i]);
                score += a[i]*times - b[i]*((times*(times-1))/2LL);
                a[i]-=times*b[i];
                score += a[i];
                a[i]=max(0LL,a[i]-b[i]);
                movesmade += times + 1;
        ...
        RSORT(a);
        movesmade = k-movesmade;
        for(int i=0;i<min(n,movesmade);i++)score+=a[i];
        cout << score << "
";
 
    }    
