...
int main(){
    ...
    TESTCASE{
        ...
        vector<ll> b;
        readvector(b,n);
        vector<vector<ll>> indices(m,vector<ll>());
        vector<ll> cnt(m,0);
        for(int i=0;i<n;i++){
            indices[b[i]].pb(i);
            cnt[b[i]]++;
...
 
        ...
        ll mod=676767677;
        for(int i=1;i<m;i++){
            for(auto c:indices[i]){
                bool neighbour=false;
                vector<ll> t;
                if(c>0 && b[c-1]<b[c]){neighbour=true;t.pb(b[c-1]);}
                if(c<n-1 && b[c+1]<b[c]){neighbour=true;t.pb(b[c+1]);}
                if(!neighbour){
                    pos=0;
                    break;
                }
